...
class Babeltrace2(AutotoolsPackage):
    ...
    variant(
        "python-plugins",
        default=False,
        description="Enable the Python plugins support for the library and converter",
    )
    ...
    variant(
        "debug-info",
        default=False,
        description="disable the debug info support (default on macOS, Solaris and Windows)",
    )
    variant("api-doc", default=False, description="Build HTML API documentation")
    variant(
        "built-in-plugins",
        default=False,
        description="Statically-link in-tree plug-ins into the babeltrace2 executable",
    )
    variant(
        "built-in-python-plugin-support",
        default=False,
        description="Statically-link Python plugin support into the babeltrace library",
    )
    variant("man-pages", default=False, description="Build man pages")
    variant(
        "asan", default=False, description="Build with AddressSanitizer", when="@2.1:"
    )
    variant(
        "ubsan",
        default=False,
    ...
    patch(
        "0001-Prevent-null-character-from-stopping-string-decoding.patch", when="@2.1:"
    )
    ...
    def configure_args(self):
        ...
        if (
            "+python-bindings" in self.spec
            or "+python-plugins" in self.spec
            or "+built-in-python-plugin-support" in self.spec
        ):
